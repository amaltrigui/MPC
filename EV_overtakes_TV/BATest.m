...
%% reference trajectory
    ...
    % assuming that the second car maintains the same velocity and same lane
    dTV         =   xTVmeasure_0(2)*ones(1,mpciterations+N+1);
    phiTV       =   zeros(1,mpciterations+N+1);
    vTV         =   xTVmeasure_0(4)*ones(1,mpciterations+N+1);
    sTV         =   (0:1:mpciterations+N)*T*xTVmeasure_0(4)+ xTVmeasure_0(1);
    x_refTV     =   [sTV;dTV;phiTV;vTV];   
...
%% Optimization
    ...
    nmpc(@runningcosts, @constraints, ...
         ...
         mpciterations, N, T, xmeasure, u0, x_ref,x_refTV,@Determine_X_new, ...
...
function [c,ceq] = constraints(x, x_ref,x_refTV, k, Properties_Obj, accident)
%% constraints
    ...
    if(accident(k)==1)
        A = x_refTV(2)+width_car+Properties_Obj.safety_distance- x(2);
        c(end)=A;
    end
...
function plotTrajectories( x, x0, x_ref, Properties_Obj,x_TV)
    ...
    % plot TV trajectory
    h9 =plot(C(:,1)+x_TV(1)-L/2,C(:,2)+x_TV(2)-B/2,'green');
