...
%% General
    ...
    writerObj=6;
    writerObj = VideoWriter('SimulationVideo2Tvsnotsamelane.avi');
    ...
    mpciterations = 70;     % the Length of the Simulation 
...
%% Initial values
    ...
    % EV:
    % Velocity of the EV is 10 m/S
    xmeasure      = [0.0; -1.5; 0.0; 15];  
    ...
    x_TV_measure  = [40; 7; -1.5; 0];  
    x_TV2_measure  = [50; 10; 1.5; 0];  
    
    nTV = 2; % Number of Target Vehicles
    xTVs = zeros(length(x_TV_measure),nTV);
    xTVs(:,1) = x_TV_measure;
    xTVs(:,2) = x_TV2_measure;
...
%% Optimization
    [~,~,totalcost]=nmpc(@runningcosts, @constraints, ...
         ...
         mpciterations, N, T, xmeasure, u0, x_ref, xTVs , @Determine_X_new, ...
            EV_system,TV_system, @computeCurvedBicycleModel, @printHeader, @printClosedloopData,...
            @plotTrajectories, @TV_prediction, @TV_dynamics, @constraint_Coeff_Genereation, writerObj, @RefrenceTV);
    totalcost
...
function TVrefrence = RefrenceTV(x_TV, T, N)
%% calculate current refernce trajectory of one TV
yTV         =   x_TV(3)*ones(1,N+1);
vyTV        =   x_TV(4)*ones(1,N+1);
vxTV        =   x_TV(2)*ones(1,N+1);
xTV         =   (0:1:N)*T*x_TV(2)+ x_TV(1);

TVrefrence    = [xTV;vxTV;yTV;vyTV];   
...
%%Switching rules Only for Left deviation (see other code for right & left)        
    ...
    if (delta_x<0 && abs(delta_x)< r_close && abs(y_EV_lane - x_TV(3,k))<1e-3 && x_TV(3,k)<0)
...
%         keyboard()
    ...
    % go inclined to right
    if (delta_x<0 && abs(delta_x)< r_close && abs(y_EV_lane - x_TV(3,k))<0.6 && x_TV(3,k)>0)
        c_1_y_ev = y_EV_lane + EV_system.Width/2;
        c_3_y_k =  x_TV(3,k) - b_r_k;
        c_1_x_ev = x_EV +  EV_system.Length/2;
        c_3_x_k =  x_TV(1,k) - a_r_k;
        qx = (c_1_y_ev-c_3_y_k)/(c_1_x_ev-c_3_x_k) ;
        qy = 1 ;
        qt = (c_1_y_ev - c_1_x_ev * qx) ;
...
%         qt = qt - 2.5*qx + qy;
        ...
        fprintf(' %11.6f %11.3f %11.3f %11.3f inclined constraint to right %11.3f %11.3f %11.3f %11.3f \n', delta_x, qx, qy, qt, c_1_y_ev, c_3_y_k,c_1_x_ev,c_3_x_k);
%         keyboard()
    ...
    if (abs(y_EV_lane - x_TV(3,k))>=1e-3 && abs(delta_x) <= r_close && x_TV(3,k)<0)
        c_2_y_k =  x_TV(3,k) + b_r_k ;
        qx = 0;
        qy = -1 ;
        qt = c_2_y_k; 
...
function plotTrajectories( x, x0, x_ref, EV_system,TV_system, xTVs, X_TV,N, writerObj, endbool)
 %% Visualize Cars 
...
%     middle_Road_3 =  middle_Road_2+middle_Road_1; % the two lanes are limited
    ...
    % plot street
    plot(linspace(-2,maxX,stepx),borders_lanes(1)*ones(stepx),'b')  % upper Borderie
    plot(linspace(-2,maxX,stepx),borders_lanes(2)*ones(stepx),'b')  % lower Borderie
...
%     
    ...
    s2 = [];
    for i=1:N    
        s1_k= plot(X_TV(1,i,1),X_TV(3,i,1),'xb', 'MarkerFaceColor','green');
        s1 = [s1 s1_k];
        s1_k= plot(X_TV(1,i,2),X_TV(3,i,2),'ob', 'MarkerFaceColor','red');
        s1 = [s1 s1_k];
...
%     b_r_k = TV_system.b_r(1);
%     a_r_k = TV_system.a_r_velocity_independent(1);

    b_r_k = B/2;
    a_r_k = L/2;
    Cnew = [0. 0. ; 2*a_r_k 0. ; 2*a_r_k  2*b_r_k  ; 0. 2*b_r_k ; 0 0.];
    
    h9 =[];
    for k=1: ntv
        h9_k = plot(Cnew(:,1)+xTVs(1,k)-a_r_k ,Cnew(:,2)+xTVs(3,k)-b_r_k,'green');
        h9 = [h9 h9_k];
    end
