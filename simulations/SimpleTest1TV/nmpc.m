function [x, u, totalcost] = nmpc(runningcosts, ...
              ...
              printHeader, printClosedloopData, plotTrajectories ,TV_prediction, TV_dynamics, constraint_Coeff_Genereation, writerObj)
...
%   EV_system: A structure  that stores the propterties of the Problem
    ...
    while(mpciter < mpciterations)
        ...
        % Reference of TV for the whole horizon N beginning from the
        % refernece at step mpciter+1 and concluding the next N references
        x_TV_ref_N = x_TV_ref(:,mpciter+1:mpciter+N+1);   
        
        %%%%%%%%%%%%%%%%%%%%%%%% Step 2: Solve the optimal control problem %%%%%%%%%%%%%%%%%%%%%%%%%%%
        
        %X_TV is now the predicted N-horizon-states of TV (without Noise)
        X_TV = TV_prediction(x_TV_0,x_TV_ref_N,N,T,w);
        
        % Update TV position  : accumulate all  states of TV from the
        % Beginning --> maybe will be usefull after
        x_TV_predicted = [ x_TV_predicted, x_TV_measure ];
        
        % TV next status (with noise)
        % nopeee--> must change later--> x_TV_measure is second element of
        % X_TV but x_TV_measure_withnoise wasnt calculated before so ... ?
        % change code hier
        [x_TV_measure_withnoise,x_TV_measure] = TV_dynamics(x_TV_0, x_TV_ref(:,mpciter+1),T, w);
        ...
        [u_new, V_current, exitflag, output] = solveOptimalControlProblem ...
            (runningcosts, constraints, ...
             linearconstraints, system, ...
             N, T, x0, u0, options, x_ref(:,mpciter+2:mpciter+N+1),X_TV(:,2:N+1),x_TV_0, ...
        ...
        % plot Cars allure
%         plotTrajectories( x_pred, x0, x_ref,EV_system,x_TV_measure, X_TV,N,writerObj , mpciter == mpciterations-1)
...
function [u, V, exitflag, output] = solveOptimalControlProblem ...
    ...
    options, x_ref,x_TV,x_TV_0, EV_system,TV_system, myMatrices, u_last,Determine_X_new, constraint_Coeff_Genereation)
    ...
    for k=1:N
        ...
        % not so sure !!!
        a_r_k = (-1/a_min_longitudinal * max([0,(x0(4)^2- x_TV(2,k)^2)])); % /2
        a_r = [a_r a_r_k];
    end
    ...
    qy = [];
    qt = [];
    delta_S_X_0 = x0(1)- x_TV_0(1);  % delta xEV0, xTV0
    
    for k= 1:N
        [qx_k, qy_k, qt_k] = constraint_Coeff_Genereation(delta_S_X_0, x0(1),x0(2), x_TV, EV_system, TV_system, k);
         qx = [qx qx_k];
         qy = [qy qy_k];
         qt = [qt qt_k];
    end
...
function [c,ceq] = nonlinearconstraints(constraints, ...
    ...
    for k=1:N
        [cnew, ceqnew] = constraints(x(:,k+1),x_ref(:,k), k, EV_system, TV_system, qx(k), qy(k), qt(k));
        c = [c cnew];
        ceq = [ceq ceqnew];
    end
